...
/*double vector:: operator()(const vector& V, const vector& M) { //скалярное произведение
	double s = 0;
	if (V.len == M.len) {
		for (int i = 0; i < V.LEN; i++) {
			s += (V.v[i],  M.v[i]);  // , M.v[i]);
...
//????????????
matrix::matrix() //придумать числа для теста
{
	n = 9; // столбцы
	N = 3;
	//array_b = 
...
matrix:: ~matrix()
{
	...
	delete[] A; // надеюсь для каждого А вызовется деструктор
	delete[] B;
	delete[] C;
}
